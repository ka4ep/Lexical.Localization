...
namespace Lexical.Localization.Tests
{
    ...
    public class DynamicTests
    {
        ...
        public void TestMethod1()
        {
            ...
            IAssetKey success = section.SetCulture("en").Success;
            ...
            Assert.AreEqual("Success", (string) section.SetCulture("en").Success);
            Assert.AreEqual(null, (byte[])section.Success.Icon);

            string inline_to_swedish_and_formulate = section.Error.SetCulture("sv").sv("Sönder (kod=0x{0:X8})").Format(0xCafeBabe);
            Assert.AreEqual("Sönder (kod=0xCAFEBABE)", inline_to_swedish_and_formulate);

            string inline_to_swedish_and_formulate_2 = section.Error.SetCulture("sv").sv("Sönder (kod=0x{0:X8}, meta={1})").Format(0xCafeBabe, "sur fisk");
            ...
            Assert.IsTrue(section.Error.fi_Savo("Onnistuepi").SetCulture("fi-Savo").Format(0xFeedF00d).Args is object[]);
        }
    }
}
