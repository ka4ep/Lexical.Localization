...
namespace Lexical.Localization
{
    ...
    public static partial class LineReaderExtensions
    {
        ...
        public static IEnumerable<KeyValuePair<string, IFormulationString>> ReadStringLines(this ILineFileFormat fileFormat, string srcFilename, ILineFormat lineFormat = default, bool throwIfNotFound = true)
        {
        ...
        public static IAsset FileAsset(this ILineFileFormat fileFormat, string filename, ILineFormat lineFormat = default, bool throwIfNotFound = true)
        {
            ...
            {
                return new LocalizationAsset().Add(fileFormat.FileReaderAsLineTree(filename, lineFormat, throwIfNotFound), lineFormat).Load();
            }
            else if (fileFormat is ILineTextReader || fileFormat is ILineStreamReader)
            {
                return new LocalizationAsset().Add(fileFormat.FileReader(filename, lineFormat, throwIfNotFound), lineFormat).Load();
            }
            else if (fileFormat is ILineStringTextReader || fileFormat is ILineStringStreamReader)
            {
                return new LocalizationAsset().Add(fileFormat.FileReaderAsStringLines(filename, lineFormat, throwIfNotFound), lineFormat).Load();
            }
        ...
        /// <returns>lines</returns>
        /// <exception cref="KeyNotFoundException">If file format was not found in <paramref name="fileFormatProvider"/></exception>
        public static IEnumerable<KeyValuePair<string, IFormulationString>> ReadStringLines(this IReadOnlyDictionary<string, ILineFileFormat> fileFormatProvider, string filename, ILineFormat lineFormat = default, bool throwIfNotFound = true)
